...
@dp.route("/manage_courses")
def manage_courses():
    lesson_topics = request.form.get("lesson_topics")

    lesson_topics = ContentCreator(lesson_topics = lesson_topics)
    db.session.add(lesson_topics)
    db.session.delete(lesson_topics)
    db.session.commit()
